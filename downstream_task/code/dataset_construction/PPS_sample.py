...
for case_id in tqdm(PPR_train):
    ...
    for neg in np.random.choice(candidate, ins_count):
        PAR_train.append((case_id, neg, 0))
...
for case_id in tqdm(PPR_test):
    ...
    for neg in np.random.choice(candidate, ins_count):
        PAR_test.append((case_id, neg, 0))
    test_pairs.add(case_id)

json.dump(PAR_train, open("../../datasets/task_2_patient2patient_similarity/PPS_train.json", "w"), indent = 4)
json.dump(PAR_dev, open("../../datasets/task_2_patient2patient_similarity/PPS_dev.json", "w"), indent = 4)
json.dump(PAR_test, open("../../datasets/task_2_patient2patient_similarity/PPS_test.json", "w"), indent = 4)
